...
import io.github.zhangjq.bcl.Constants.CHARGE_LIMIT_ENABLED
import io.github.zhangjq.bcl.Constants.CHARGE_OFF_KEY
import io.github.zhangjq.bcl.Constants.CHARGE_ON_KEY
import io.github.zhangjq.bcl.Constants.CHARGING_CHANGE_TOLERANCE_MS
import io.github.zhangjq.bcl.Constants.DEFAULT_DISABLED
import io.github.zhangjq.bcl.Constants.DEFAULT_ENABLED
import io.github.zhangjq.bcl.Constants.DEFAULT_FILE
import io.github.zhangjq.bcl.Constants.FILE_KEY
...
//        }
    ...
    fun setLimit(limit: Int, settings: SharedPreferences) {
        // keep the recharge threshold the user set, instead of overwriting it
        // with a recalculated value that would silently change the behaviour
        val currentMin = settings.getInt(MIN, -1)
        val newMin = when {
            currentMin < 0 -> (limit - 2).coerceAtLeast(0)      // never set yet, use a default
            currentMin >= limit -> (limit - 2).coerceAtLeast(0) // must stay below the new max
            else -> currentMin                                   // preserve the user's value
        }
        settings.edit().putInt(LIMIT, limit).putInt(MIN, newMin).apply()
    }
    ...
    /**
     * Shut down the device using root. Tries `reboot -p` first and falls back to
     * `svc power shutdown` if the reboot binary is not available.
     */
    fun shutdownDevice(onFailed: (() -> Unit)? = null) {
        Shell.cmd("reboot -p").submit {
            if (!it.isSuccess) {
                Log.w(TAG, "reboot -p failed, trying svc power shutdown")
                Shell.cmd("svc power shutdown").submit { result ->
                    if (!result.isSuccess) {
                        Log.e(TAG, "svc power shutdown failed")
                        onFailed?.invoke()
                    }
                }
            }
    ...
    fun startAutoShutdownServiceIfEnabled(context: Context) {
        if (!getPrefs(context).getBoolean(PrefsFragment.KEY_AUTO_SHUTDOWN_ENABLED, false)) {
            return
        }
        try {
            context.startService(
                Intent(context, AutoShutdownService::class.java)
                    .setAction(Constants.ACTION_AUTO_SHUTDOWN_EVALUATE)
            )
        } catch (e: Exception) {
            Log.e(TAG, "Could not start auto shutdown service", e)
        }
    }

    fun stopAutoShutdownService(context: Context) {
        AutoShutdownService.cancelScheduledWork(context)
        context.stopService(Intent(context, AutoShutdownService::class.java))
    }
    ...
    fun stopService(context: Context, ignoreAutoReset: Boolean = true) {
        ...
        // display the matching Toast message if not disabled in settings
